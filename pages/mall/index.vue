<template>
	...
	<view class="list">
		...
		<view class="tips" :class="{ 'tips-ani': tipShow }">更新成功</view>
		<!-- 页面分类标题 -->
		...
		<uni-list :class="{ 'uni-list--waterfall': options.waterfall }">
			...
			<uni-list-item :border="!options.waterfall" class="uni-list-item--waterfall" title="自定义商品列表"
				v-for="item in product_data" :key="item._id"
				:to="'/pages/mall/detail?id='+item._id+'&title='+item.name">
				<!-- 通过header插槽定义列表左侧图片 -->
...
<script>
	...
	export default {
		...
		methods: {
			 ...
			 */
			select() {
				this.options.waterfall = !this.options.waterfall;
			},
			...
			loadData(options, callback) {
				this.loading = true;
				axios.get('/mock/mall/mall.json')
					.then((response) => {
						// let category_id = nanoid();
						let new_product_data = [];
						let fields = this.field.split(",");
						response.data.mall.product.forEach(item => {
							item._id = nanoid();
							// item.category_id = category_id; // 暂时不需要
							let product = {
								_id: item._id,
							};
							...
							new_product_data.push(product);
						});
						if (options && options.clear === true) {
							this.product_data.length = 0;
							this.product_data = new_product_data;
						} else {
							this.product_data = this.product_data.concat(new_product_data);
						}
					})
					.catch((error) => {
						console.log(error);
